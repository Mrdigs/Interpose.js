...
interpose.init = function(start, end) {
	...
	return function(template) {
		...
		for(index = 0; index < items.length; index++) {
			var part = items[index], even = index % 2 == 0, interpolate = !even && part.trim().charAt(0) == '=', item = even || interpolate ? part.replace(/"/g, '\\"') : part;
			code += (even ? 'echo("' + item + '")' : interpolate ? 'print(' + item.replace('=', '') + ')' : item) + ';';
		}
		code += 'return o.join("");';
